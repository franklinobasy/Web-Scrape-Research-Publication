...
class Extract():
    ...
    '''
    __url: str = "https://pureportal.coventry.ac.uk/en/organisations/school-of-economics-finance-and-accounting/publications/"
    __pubs: List[dict] = []
    __headers: dict = {
        ...
        for page in range(self._page_to_parse):
            ...
            for item in soup.find_all("li", class_="list-result-item"):
                ...
                for pub in item.find_all("a", class_="link person"):
                    ...
                    pub_object[f"author_{i}"] = author
                    pub_object[f"author_{i}_profile"] = link
                ...
                for pub in item.find_all("span", class_="")[1:-1]:
                    if not Extract.check_author_in_author_links(authors_with_link, pub.text):
                        pub_object[f"author_{i}"] = pub.text
